...
@pytest.mark.asyncio
async def test_geo_by_ip(async_client: httpx.AsyncClient, respx_mock: respx.MockRouter) -> None:
    payload = {
        "status": "success",
        "query": "8.8.8.8",
...
@pytest.mark.asyncio
async def test_geo_for_client_uses_forwarded_for(
    ...
    response = await async_client.get("/v1/geo", headers={"x-forwarded-for": "1.1.1.1"})
...
@pytest.mark.asyncio
async def test_geo_by_ip_invalid(async_client: httpx.AsyncClient) -> None:
